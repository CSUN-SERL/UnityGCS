...
namespace Missions.ManualControl
{
    public class ManualControlButton : MonoBehaviour
    {
        ...
        private void OnApplicationQuit()
        {
            ...
            if (Isbot1.isOn && flag1 == false)
                GCSSocket.Instance.socket.Emit(ServerURL.TOGGLE_MANUAL_CONTROL,
                    1);
        ...
        {
            if (Isbot1.isOn && flag1 == false)
                GCSSocket.Instance.socket.Emit(GCSSocket.TOGGLE_MANUAL_CONTROL, 1);
            if (Isbot2.isOn && flag2 == false)
                GCSSocket.Instance.socket.Emit(GCSSocket.TOGGLE_MANUAL_CONTROL, 1);
            if (Isbot3.isOn && flag3 == false)
                GCSSocket.Instance.socket.Emit(GCSSocket.TOGGLE_MANUAL_CONTROL, 1);
    ...
    */
        ...
        if (CurrentRobot == robotId)
        {
            _topic = "";
            CurrentRobot = -1;
            GCSSocket.Instance.socket.Emit(ServerURL.TOGGLE_MANUAL_CONTROL, CurrentRobot);
            Debug.Log(robotId);
