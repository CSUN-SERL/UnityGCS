...
namespace AudioStream
{
    public class AudioSourceOutputDevice : MonoBehaviour
    {
        ...
        #endregion
        [Header("[Advanced]")]
        [Tooltip("Keep DSP buffer size as small as possible for small latency, and possibly a power of 2.\r\nThe default (64) seems to be working with Best Latency Unity audio setting.")]
        public uint dspBufferSize = 64;
        [Tooltip("Keep the count as small as possible for best latency.")]
        public int dspBuffersCount = 2;
        /// <summary>
        ...
        FMOD.RESULT PCMReadCallback(System.IntPtr soundraw, System.IntPtr data, uint datalen)
        {
            ...
            lock (this.pcmReadCallbackBufferLock)
            {
                var count_available = this.pcmReadCallbackBuffer[this.pcmReadCallback_ActiveBuffer].Count;
                var count_provide = (int)Mathf.Min(count_available, (int)datalen);

                LOG(LogLevel.DEBUG, "PCMReadCallback requested {0} while having {1} (providing {2}),  time: {3}", datalen, count_available, count_provide, AudioSettings.dspTime);

                if (count_provide > 0)
                {
                    var bArr = this.pcmReadCallbackBuffer[this.pcmReadCallback_ActiveBuffer].GetRange(0, count_provide).ToArray();

                    System.Runtime.InteropServices.Marshal.Copy(bArr, 0, data, count_provide);

                    int count_rest = count_available - count_provide;

                    if (count_rest > 0)
                        this.pcmReadCallbackBuffer[1 - this.pcmReadCallback_ActiveBuffer].AddRange(this.pcmReadCallbackBuffer[this.pcmReadCallback_ActiveBuffer].GetRange(count_provide, count_rest));
                ...
                else
                {
                    ...
                    System.Array.Clear(bArr, 0, (int)datalen);
        ...
        void CreateAndInitSystem(bool syncOutput)
        {
            ...
            */
            ...
            /*
             * Try to adjust DSP buffers in order to reduce latency
             */
            uint bufferLength;
            int numBuffers;
            ...
            result = system.setDSPBufferSize(this.dspBufferSize, this.dspBuffersCount);
